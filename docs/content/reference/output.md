...
describes what users get. Pick a format with `-o`, or let sbdi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
sbdi <command> -o table   # aligned columns for reading
sbdi <command> -o jsonl   # one JSON object per line, for piping
sbdi <command> -o json    # a single JSON array
sbdi <command> -o csv     # spreadsheet friendly
sbdi <command> -o tsv     # tab-separated
sbdi <command> -o url     # just the URL column
sbdi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
sbdi <command> --fields id,title,url
```
...
```bash
sbdi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
sbdi <command>            # a table, because this is a terminal
sbdi <command> | wc -l    # JSONL, because this is a pipe
```
